...
namespace Week4_05_hansohee
{
    public partial class Form1 : Form
    {
        ...
        private void btnInsert_Click(object sender, EventArgs e)
        {
            ...
            {
                lblOutput.Text = "학번을 반드시 넣어 주세요";
                tbxNumber.Focus();
                return;
            }
            if (string.IsNullOrEmpty(tbxName.Text))
            {
                lblOutput.Text = "이름을 반드시 넣어 주세요";
                tbxName.Focus();
                return;
            }
            ...
            {
                lblOutput.Text = "숫자를 넣어 주세요!";
                return;
            }

            if (kor < 0.0 || kor > 100.0 || eng < 0.0 || eng > 100.0
                || mat < 0.0 || mat > 100.0)
            {
                lblOutput.Text = "0~100 사이의 점수를 넣어!";
                return;
            }
            ...
            lblOutput.Text += $"국어 : {kor} 영어 : {eng} 수학 : {mat}이 입력 완료!!!";
        ...
        private void btnView_Click(object sender, EventArgs e)
        {
            ...
            lblOutput.Text = $"총 입력 학생 수 : {index}명";
            lblOutput.Text += Environment.NewLine;

            foreach (Student stu in students) // for (int i = 0; i < index; i++)  // 학생 수 0~9
            {
                if (stu == null)
                {
                    continue;
                ...
                for (int j = 0; j < stu.Scores.Length; j++) // 과목 0~2
                {
                    lblOutput.Text += $" {stu.Scores[j],8:F1}점 ";
                    sum_stu += stu.Scores[j];
                }
                avg_stu = sum_stu / stu.Scores.Length;
                lblOutput.Text += $"평균 : {avg_stu,8:F1}";
                lblOutput.Text += Environment.NewLine;

                sum += avg_stu;
                if (max < avg_stu) max = avg_stu;
                if (min > avg_stu) min = avg_stu;

            }

            avg = sum / index;

            lblOutput.Text += $"전체 평균 : {avg:F2} 최고점 : {max:F2} 최저점 : {min:F2}";
        }
    }
}
